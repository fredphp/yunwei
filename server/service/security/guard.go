...
func (g *SecurityGuard) GetSecurityStats(days int) map[string]int64 {
	...
	// 事件统计
	global.DB.Model(&SecurityEvent{}).Where("created_at > ?", since).Count(&stats["totalEvents"])
	global.DB.Model(&SecurityEvent{}).Where("created_at > ? AND level = ?", since, SecurityLevelCritical).Count(&stats["criticalEvents"])
	global.DB.Model(&SecurityEvent{}).Where("created_at > ? AND level = ?", since, SecurityLevelHigh).Count(&stats["highEvents"])

	// 登录统计
	global.DB.Model(&LoginRecord{}).Where("created_at > ?", since).Count(&stats["totalLogins"])
	global.DB.Model(&LoginRecord{}).Where("created_at > ? AND success = ?", since, true).Count(&stats["successLogins"])
	global.DB.Model(&LoginRecord{}).Where("created_at > ? AND success = ?", since, false).Count(&stats["failedLogins"])
	global.DB.Model(&LoginRecord{}).Where("created_at > ? AND is_abnormal = ?", since, true).Count(&stats["abnormalLogins"])

	// 黑名单统计
	global.DB.Model(&IPBlacklist{}).Where("enabled = ?", true).Count(&stats["blacklistedIPs"])
	global.DB.Model(&IPBlacklist{}).Where("enabled = ? AND auto_banned = ?", true, true).Count(&stats["autoBannedIPs"])
