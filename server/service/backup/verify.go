...
func (s *VerifyService) verifyRestoreTest(ctx context.Context, filePath string, record *backup.BackupRecord) VerifyCheckItem {
	...
	result, err := restoreSvc.restoreFilesystem(ctx, record, target, data, config, &RestoreResult{}, &strings.Builder{})
	if err != nil {
		check.Status = "failed"
	...
	if result != nil {
		check.Status = "passed"
		check.Message = fmt.Sprintf("恢复测试成功，恢复 %d 个文件", result.RestoredFiles)
	} else {
		check.Status = "warning"
