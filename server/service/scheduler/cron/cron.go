...
func (cs *CronScheduler) executeJob(jobID uint, entry *CronJobEntry) {
	...
	case "forbid":
		...
		global.DB.Model(&scheduler.Task{}).Where(
			"status = ? AND json_extract(params, '$.cron_job_id') = ?",
			scheduler.TaskStatusRunning, jobID,
		).Count(&runningCount)
		if runningCount > 0 {
			return
		}
	...
	case "replace":
		...
		for _, t := range runningTasks {
			t.Status = scheduler.TaskStatusCanceled
			global.DB.Save(&t)
		}
	}
	
	// 创建执行记录
	execution := &scheduler.CronExecution{
		CronJobID:   jobID,
	...
	if err := parseJSON(job.TaskTemplate, &taskDef); err != nil {
	...
	if taskType, ok := taskDef["type"].(string); ok {
		task.Type = scheduler.TaskType(taskType)
	}
	...
	execution.Status = scheduler.TaskStatusRunning
	now := time.Now()
	...
	// 记录事件
	scheduler.RecordTaskEvent(task.ID, "cron_triggered", map[string]interface{}{
		"cron_job_id": jobID,
		"scheduled":   entry.PrevRun,
	}, "cron", "Cron 任务触发")
}

// AddJob 添加任务
func (cs *CronScheduler) AddJob(job *scheduler.CronJob) error {
	// 解析 Cron 表达式
...
// UpdateJob 更新任务
func (cs *CronScheduler) UpdateJob(job *scheduler.CronJob) error {
	cs.RemoveJob(job.ID)
	if job.Enabled {
...
func CreateCronJob(job *scheduler.CronJob) error {
...
func GetCronJob(id uint) (*scheduler.CronJob, error) {
...
func GetCronJobs() ([]scheduler.CronJob, error) {
	var jobs []scheduler.CronJob
...
func UpdateCronJob(job *scheduler.CronJob) error {
	return global.DB.Save(job).Error
}

// DeleteCronJob 删除 Cron 任务
func DeleteCronJob(id uint) error {
	return global.DB.Delete(&scheduler.CronJob{}, id).Error
}

// EnableCronJob 启用 Cron 任务
func EnableCronJob(id uint) error {
	return global.DB.Model(&scheduler.CronJob{}).Where("id = ?", id).Update("enabled", true).Error
}

// DisableCronJob 禁用 Cron 任务
func DisableCronJob(id uint) error {
	return global.DB.Model(&scheduler.CronJob{}).Where("id = ?", id).Update("enabled", false).Error
}
...
func GetCronExecutions(cronJobID uint, limit int) ([]scheduler.CronExecution, error) {
	var executions []scheduler.CronExecution
