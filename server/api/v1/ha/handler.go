...
var (
	haManager     *haService.HAManager
	haManagerOnce sync.Once
)

// getHAManager 获取 HA 管理器实例（延迟初始化）
func getHAManager() *haService.HAManager {
	haManagerOnce.Do(func() {
		haManager = haService.NewHAManager()
	})
	return haManager
}
...
func GetClusterNodes(c *gin.Context) {
	...
	nodes, total, err := getHAManager().GetClusterManager().ListNodes(filter)
	if err != nil {
...
func GetClusterNode(c *gin.Context) {
	...
	node, err := getHAManager().GetClusterManager().GetNode(nodeID)
	if err != nil {
...
func EnableNode(c *gin.Context) {
	...
	if err := getHAManager().GetClusterManager().EnableNode(nodeID); err != nil {
...
func DisableNode(c *gin.Context) {
	...
	if err := getHAManager().GetClusterManager().DisableNode(nodeID); err != nil {
...
func GetNodeMetrics(c *gin.Context) {
	...
	metrics, err := getHAManager().GetClusterManager().GetNodeMetrics(nodeID, hours)
	if err != nil {
...
func GetLeaderStatus(c *gin.Context) {
	status := gin.H{
		"nodeId":    getHAManager().GetNodeID(),
		"isLeader":  getHAManager().IsLeader(),
		"leader":    getHAManager().GetLeader(),
		"term":      getHAManager().GetLeaderService().GetTerm(),
		"timestamp": time.Now(),
	}
...
func ResignLeader(c *gin.Context) {
	if !getHAManager().IsLeader() {
		response.FailWithMessage("当前不是 Leader", c)
		return
	}

	if err := getHAManager().GetLeaderService().Resign(); err != nil {
...
func ForceLeader(c *gin.Context) {
	...
	if err := getHAManager().GetLeaderService().ForceLeader(req.NodeID); err != nil {
...
func GetLocks(c *gin.Context) {
	...
	locks, total, err := getHAManager().GetLockService().ListLocks(filter)
	if err != nil {
...
func GetLock(c *gin.Context) {
	...
	info, err := getHAManager().GetLockService().GetLockInfo(key)
	if err != nil {
...
func ForceReleaseLock(c *gin.Context) {
	...
	if err := getHAManager().GetLockService().ForceRelease(nil, key, req.Reason); err != nil {
...
func GetSessions(c *gin.Context) {
	...
	sessions, total, err := getHAManager().GetSessionManager().ListSessions(filter)
	if err != nil {
...
func DeleteSession(c *gin.Context) {
	...
	if err := getHAManager().GetSessionManager().DeleteSession(nil, sessionID); err != nil {
...
func GetSessionStats(c *gin.Context) {
	stats := getHAManager().GetSessionManager().GetSessionStats()
	response.OkWithData(stats, c)
}
...
func UpdateHAConfig(c *gin.Context) {
	...
	if err := getHAManager().UpdateConfig(&config); err != nil {
...
func GetFailoverRecords(c *gin.Context) {
	...
	records, total, err := getHAManager().GetFailoverRecords(filter)
	if err != nil {
...
func TriggerFailover(c *gin.Context) {
	...
	if err := getHAManager().TriggerFailover(req.NodeID); err != nil {
...
func GetClusterEvents(c *gin.Context) {
	...
	events, total, err := getHAManager().GetEvents(filter)
	if err != nil {
...
func GetRunningTasks(c *gin.Context) {
	tasks := getHAManager().GetTaskHAManager().GetRunningTasks()
